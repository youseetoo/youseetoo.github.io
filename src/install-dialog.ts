...
// UC2 Configuration interfaces
interface UC2ConfigOption {
  label: string;
  description?: string;
  command: string;
...
interface UC2Manifest extends Manifest {
  uc2_config?: UC2ConfigOption[];
}
...
export class EwtInstallDialog extends LitElement {
  ...
  protected render() {
    ...
    } else if (this._state === "UC2_CONFIG") {
      [heading, content] = this._renderUC2Config();
    }
        ...
        <ew-list>
          ...
          ${this._manifest.uc2_config
            ? html`
                ...
                >
                  <div slot="start" class="fake-icon">⚙️</div>
                  <div slot="headline">UC2 Configuration</div>
                  <div slot="supporting-text">Configure device-specific settings</div>
                </ew-list-item>
              `
            : ""}
  ...
  _renderUC2Config(): [string | undefined, TemplateResult] {
    const heading = "UC2 Configuration";
    let content: TemplateResult;

    if (!this._manifest.uc2_config || this._manifest.uc2_config.length === 0) {
      content = html`
        <div slot="content">
          <ewt-page-message
            .icon=${"❌"}
            label="No configuration options available for this device."
          ></ewt-page-message>
        </div>
        <div slot="actions">
          <ew-text-button
    ...
    content = html`
      <div slot="content">
        <div>Configure device-specific settings:</div>
        <ew-list>
          ${this._manifest.uc2_config.map((config) => {
            if (config.type === "button") {
              return html`
                <ew-list-item
                  type="button"
                  @click=${() => this._executeUC2Command(config.command)}
                >
                  <div slot="headline">${config.label}</div>
                  ${config.description
                    ? html`<div slot="supporting-text">${config.description}</div>`
                    : ""}
                </ew-list-item>
              `;
              ...
              return html`
                <ew-list-item>
                  ...
                  <ew-filled-select
                    slot="end"
                    @change=${(ev: { target: EwFilledSelect }) => {
                      const selectedOption = config.options![ev.target.selectedIndex || 0];
                      const command = config.command.replace(
                        "{value}",
                        selectedOption.value.toString()
                      );
                      this._executeUC2Command(command);
                    }}
                  >
                    ${config.options.map(
                      (option) => html`
                        <ew-select-option .value=${option.value}>
                          ${option.label}
                        </ew-select-option>
                      `
                    )}
  ...
  private async _executeUC2Command(command: string) {
    try {
